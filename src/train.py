...
# Split data first: To actually show learning
# Did this inside of the NN as well.
train_size = int(0.9 * len(encoded_txt))
train_data = encoded_txt[:train_size]
test_data = encoded_txt[train_size:]


# ============================================
# PARAMETER INITIALIZATION (ONCE, BEFORE LOOPS)
# ============================================
# These parameters must be initialized OUTSIDE the training loop.
# If we reset them every iteration, the model will never learn!
...
# TRAINING LOOP
# ============================================
# Have to start the training Loop here. Each time a new
# batch must be sampled!
num_epochs = 100
...
for epoch in range(num_epochs):
    ...
    for step in range(steps_per_epoch):
        ...
        # ------------------------------------------------------------
        # 6.1 Backprop through out = attention @ V_att
        # ------------------------------------------------------------
        #
        # out[b] = attention[b] @ V_att[b]
        # So:
        #   dattention[b] = dout[b] @ V_att[b].T
        #   dV_att[b]     = attention[b].T @ dout[b]

        V_T = np.transpose(V_att, (0, 2, 1))                 # (B, H, T)
    ...
    # Every 2 epochs, generate some text to show learning progress
    if (epoch + 1) % 2 == 0:
        print("  Sample generation:")
        
        # Start with a seed sequence (first T tokens from training data)
        seed = train_data[:T].copy()
        generated = seed.tolist()
        
        # Generate 50 more tokens
        for _ in range(50):
            # Prepare input (last T tokens)
            context = np.array(generated[-T:]).reshape(1, T)
            
            # Forward pass (simplified - just for generation)
            tok = token_embedding_table[context]
            pos = positional_embedding_table[positions]
            x_seq = tok + pos
            ...
            # Sample from the distribution (with temperature for diversity)
            temperature = 0.8
            probs_gen = np.exp(last_logits / temperature)
            probs_gen = probs_gen / np.sum(probs_gen)
            
            # Sample next token
            next_token = np.random.choice(V, p=probs_gen)
...
# ========================================================
# After training, we can generate longer sequences to see
# what patterns the model has learned from the text.
#
# The generation process is autoregressive:
#   1. Start with a seed sequence (length T)
#   2. Run forward pass to get logits for next token
#   3. Sample from the probability distribution
#   4. Append the new token
#   5. Repeat using the last T tokens as context
#
# Temperature controls randomness:
#   - Low (0.1-0.5): More deterministic, repetitive
#   - Medium (0.7-1.0): Balanced creativity
#   - High (>1.0): More random, less coherent

print("\nFinal text generation (200 tokens):")
print("=" * 50)
seed = train_data[:T].copy()
generated = seed.tolist()
